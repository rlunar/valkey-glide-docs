---
title: Publish and Subscribe Messages
description: See how to subscribe and publish messages with GLIDE
---
...
<Tabs syncKey="progLangInExamples">
  ...
  <TabItem label="Php">
    ```php
    // Note: PHP GLIDE does not support polling/async message retrieval
    // Only callback-based PubSub is available (same as "with callback" example)
    
    // See the "Subscribe with callback" example above for PHP usage
    ```
  </TabItem>
</Tabs>
...
To learn more about GLIDE's PubSub model, see our [explanation](/concepts/pubsub-model).